...
const skipToPreviousArgsSchema = {
...
type SkipToPreviousArgs = {
    device_id?: string;
};

export class SkipToPreviousTool implements ITool<typeof skipToPreviousArgsSchema> {
    name = 'skip_to_previous';
    description = 'Skips to previous track in the user\'s queue. This API only works for users who have Spotify Premium.';
    argsSchema = skipToPreviousArgsSchema;

    async execute(args: SkipToPreviousArgs, { authInfo }: RequestHandlerExtra<ServerRequest, ServerNotification>): Promise<CallToolResult> {
        if (!authInfo) {
            return {
                content: [
                    {
        ...
        try {
            ...
            if (!deviceId) {
                try {
                    ...
                    // If we can't get playback state, try to skip anyway without device_id
                    deviceId = undefined;
                }
            } else {
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Skipped to previous track successfully${deviceInfo}.`,
                    },
                ],
            };
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Error skipping to previous track: ${error instanceof Error ? error.message : 'Unknown error'}`,
                    },
                ],
            };
        }
...
toolsRegistry.register(new SkipToPreviousTool());
