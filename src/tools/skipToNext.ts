...
export class SkipToNextTool implements ITool<typeof skipToNextArgsSchema> {
    name = 'skip_to_next';
    description = 'Skips to next track in the user\'s queue. This API only works for users who have Spotify Premium.';
    argsSchema = skipToNextArgsSchema;

    async execute(args: SkipToNextArgs, { authInfo }: RequestHandlerExtra<ServerRequest, ServerNotification>): Promise<CallToolResult> {
        if (!authInfo) {
            return {
                content: [
                    {
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Skipped to next track successfully${deviceInfo}.`,
                    },
                ],
            };
            ...
            if (error instanceof Error) {
                ...
                if (error.message.includes('No more tracks')) {
                    return {
                        content: [
                            {
                                type: 'text',
            ...
            return {
                content: [
                    {
                        type: 'text',
                        text: `Error skipping to next track: ${error instanceof Error ? error.message : 'Unknown error'}`,
                    },
                ],
            };
        }
...
toolsRegistry.register(new SkipToNextTool());
